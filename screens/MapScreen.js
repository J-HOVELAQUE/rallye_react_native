...
function MapScreen(props) {
        ...
        <Overlay overlayStyle={{
            ...
            <View style={{ width: '90%' }}>
                <Text style={{ textAlign: 'center' }}>
                    Créez un compte ou connectez vous pour accéder à la carte
                </Text>
                <RedButton onPress={() => {
    ...
    const overlayFavorites = (
        <Overlay overlayStyle={{
            ...
            isVisible={visibleFavorites}
            onBackdropPress={() => setVisibleFavorites(!visibleFavorites)}>
            <View style={{ width: '90%' }}>
                <Text style={{ textAlign: 'center' }}>
                    Ajoutez des favoris pour les suivre sur la carte
                </Text>
                <RedButton onPress={() => {
    ...
    useEffect(() => {
        // Open web socket for receiving data from server //
        socket.on('sendPositionToAll', (msg) => {
            setVehiculeToDisplay(msg.allPosition)
        })

        // Displaying overlay if user isn't connected or haven't any favorite //
